"""Idempotency proof for Phase 3 staged mock spine (ticket-151)."""
...
TEST_QUESTION_ID = "rq_staged_idempotency_spine"
...
STAGED_RUN_ID = "run_staged_phase3_idempotency"
STAGED_TOPIC = "Human-AI co-creativity idempotency (staged Phase 3 spine)"
...
def temp_db(tmp_path: Path) -> Path:
    return tmp_path / "staged_idempotency.sqlite"
...
def _run_full_spine(
    ...
    with patch(
        ...
        _mock_html_urlopen(SAMPLE_HTML),
    ):
...
def test_staged_phase3_full_spine_twice_is_idempotent(
...
def test_staged_phase3_per_command_reruns_are_idempotent(
    mock_network_env: None,
    temp_db: Path,
    staging_dir: Path,
    report_dir: Path,
