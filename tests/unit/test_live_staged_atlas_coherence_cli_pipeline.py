"""Opt-in live network + atlas coherence CLI pipeline (ticket-291).

Chains staged orchestrator → ``export-atlas-snapshot`` CLI → ``atlas-coherence-report`` CLI
on temp paths (mock LLM upstream).
...
python -m pytest tests/unit/test_live_staged_atlas_coherence_cli_pipeline.py -m live_network -q
```
"""
...
TEST_QUESTION_ID = "rq_live_staged_atlas_coherence_cli_pipeline"
STAGED_RUN_ID = "run_live_staged_atlas_coherence_cli_pipeline"
STAGED_TOPIC = "Live Atlas coherence CLI pipeline proof"


def require_live_staged_atlas_coherence_cli_pipeline_env() -> None:
    allow = os.environ.get("RGE_ALLOW_LIVE_STAGED_ORCHESTRATOR", "0").strip().casefold()
    if allow not in ("1", "true", "yes"):
        pytest.skip(
            "live atlas coherence CLI pipeline requires "
    ...
    if network not in ("1", "true", "yes"):
        pytest.skip(
            "live atlas coherence CLI pipeline requires RGE_ALLOW_SOURCE_NETWORK=1"
        )
    if not os.environ.get("OPENALEX_MAILTO", "").strip():
        pytest.skip("live atlas coherence CLI pipeline requires OPENALEX_MAILTO")


def test_require_live_staged_atlas_coherence_cli_pipeline_skips_without_opt_in(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("RGE_ALLOW_LIVE_STAGED_ORCHESTRATOR", raising=False)
    with pytest.raises(pytest.skip.Exception):
        require_live_staged_atlas_coherence_cli_pipeline_env()
...
def temp_db(tmp_path: Path) -> Path:
    return tmp_path / "live_atlas_coherence_cli_pipeline.sqlite"
...
def test_live_staged_orchestrator_export_and_coherence_cli_pipeline(
    live_env: None,
    temp_db: Path,
    staging_dir: Path,
    report_dir: Path,
    ...
    """Orchestrator → export-atlas-snapshot CLI → atlas-coherence-report CLI."""
    require_live_staged_atlas_coherence_cli_pipeline_env()

    probe_db = tmp_path / "coherence_cli_pipeline_probe.sqlite"
    ...
    assert coherence_payload["overall_coherence_verdict"] in {"pass", "partial", "fail"}
            ...
            json.dumps(
                {
                    ...
                    "coherence_cli": coherence_payload,
                },
                indent=2,
            )
        )
