"""Phase 3 spine through link-concepts on staged-ingested source (ticket-145)."""
...
from rge.modules.concept_linker import _default_link_fixture_for_source

REPO_ROOT = Path(__file__).resolve().parents[2]
OPENALEX_FIXTURE = REPO_ROOT / "fixtures" / "source_providers" / "openalex_works_sample.json"
TEST_QUESTION_ID = "rq_staged_link_spine"
...
def temp_db(tmp_path: Path) -> Path:
    return tmp_path / "staged_link_spine.sqlite"
...
def test_default_link_fixture_for_staged_spine_source() -> None:
    class _Source:
        title = STAGED_TITLE
        source_type = "peer_reviewed_empirical"

    assert _default_link_fixture_for_source(_Source()) == "staged_fetch_link_concepts.json"


def test_staged_discover_fetch_ingest_extract_link_spine(
...
def test_link_concepts_explicit_staged_fixture(
    temp_db: Path,
    staging_dir: Path,
    mock_network_env: None,
) -> None:
    ...
    assert main(
        [
            ...
            source_id,
            "--fixture",
            "staged_fetch_link_concepts.json",
            "--db",
            str(temp_db),
        ]
    ) == 0
    ...
    try:
        ...
        assert "co-creation" in linked_labels
    finally:
        conn.close()
