"""Opt-in live network + live Ollama staged build spine (ticket-212).

Default pytest collection excludes ``live_network`` and ``live_smoke`` tests.

Operator opt-in (real OpenAlex HTTP through mock extract + mock link; live Ollama build only):
...
TEST_QUESTION_ID = "rq_live_staged_build_live_llm_spine"
...
class _StubStagedBuildOllamaClient:
    provider = "ollama"
    model = "stub-qwen"
...
def require_live_staged_build_live_llm_env() -> None:
    """Skip unless operator explicitly opts into live staged Ollama build."""
    allow = os.environ.get("RGE_ALLOW_LIVE_STAGED_BUILD_LIVE_LLM", "0").strip().casefold()
    if allow not in ("1", "true", "yes"):
        pytest.skip("live staged build requires RGE_ALLOW_LIVE_STAGED_BUILD_LIVE_LLM=1")
    live = os.environ.get("RGE_ALLOW_LIVE_LLM", "0").strip().casefold()
    if live not in ("1", "true", "yes"):
        pytest.skip("live staged build requires RGE_ALLOW_LIVE_LLM=1")
    if os.environ.get("RGE_LLM_MODE", "mock") != "ollama":
        pytest.skip("live staged build requires RGE_LLM_MODE=ollama")
...
def test_require_live_staged_build_live_llm_skips_without_opt_in(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("RGE_ALLOW_LIVE_STAGED_BUILD_LIVE_LLM", raising=False)
    with pytest.raises(pytest.skip.Exception):
        require_live_staged_build_live_llm_env()
...
def temp_db(tmp_path: Path) -> Path:
    return tmp_path / "live_staged_build_live.sqlite"
...
def test_staged_source_without_fallthrough_still_uses_auto_mock_build_fixture(
    temp_db: Path,
    staging_dir: Path,
...
def test_live_staged_build_fallthrough_blocked_without_staged_live_gate(
    temp_db: Path,
    staging_dir: Path,
    staged_db_with_candidate: Path,
) -> None:
    ...
    with patch.dict(
        ...
        os.environ.pop("RGE_ALLOW_LIVE_STAGED_BUILD_LIVE_LLM", None)
        exit_code = main(
            [
                "build-relationships",
                "--source",
...
def test_live_staged_build_fallthrough_blocked_on_default_graph_db(
    temp_db: Path,
    staging_dir: Path,
    staged_db_with_candidate: Path,
) -> None:
...
def test_mocked_live_staged_build_fallthrough_persists_validated_relationships(
    temp_db: Path,
    staging_dir: Path,
    staged_db_with_candidate: Path,
) -> None:
    ...
    with patch.dict(
        ...
        with patch(
            ...
            with patch(
                ...
                return_value=_StubStagedBuildOllamaClient(),
            ):
                exit_code = main(
                    [
                        "build-relationships",
...
@pytest.fixture()
def live_staged_build_live_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("RGE_ALLOW_LIVE_STAGED_BUILD_LIVE_LLM", "1")
...
def test_live_openalex_discover_through_live_build(
    live_staged_build_live_env: None,
    temp_db: Path,
    staging_dir: Path,
    ...
    assert build_payload.get("live_staged_build_fallthrough") is True
