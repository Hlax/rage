"""Opt-in live network + live Ollama staged extract spine (ticket-204).

Default pytest collection excludes ``live_network`` and ``live_smoke`` tests.

Operator opt-in (real OpenAlex HTTP through ingest; live Ollama extract only):
...
TEST_QUESTION_ID = "rq_live_staged_extract_live_llm_spine"
CANDIDATE_ID = "disc_openalex_W2741809807"
REFERENCE_YEAR = 2026
STAGED_CHUNK_QUOTE = "Human-AI co-creativity supports diverse songwriting outputs."


class _StubStagedOllamaClient:
    provider = "ollama"
    model = "stub-qwen"
        ...
        return CandidateClaimBatch_v0_1.model_validate(
            {
                ...
                "items": [
                    {
                        "claim_text": STAGED_CHUNK_QUOTE,
                        "quote_span": STAGED_CHUNK_QUOTE,
                        "scope": "diverse songwriting outputs",
                        "subject": "Human-AI co-creativity",
                        "predicate": "supports",
                        "object": "diverse songwriting outputs",
                    ...
                    {
                        "claim_text": "Human-AI co-creativity always improves originality.",
                        "quote_span": None,
                        "scope": "all creative tasks",
                        "subject": "Human-AI co-creativity",
                        "predicate": "improves",
                        "object": "originality",
                        "evidence_type": "empirical",
...
def require_live_staged_extract_live_llm_env() -> None:
    """Skip unless operator explicitly opts into live staged Ollama extract."""
    allow = os.environ.get("RGE_ALLOW_LIVE_STAGED_EXTRACT_LIVE_LLM", "0").strip().casefold()
    if allow not in ("1", "true", "yes"):
        pytest.skip("live staged extract requires RGE_ALLOW_LIVE_STAGED_EXTRACT_LIVE_LLM=1")
    live = os.environ.get("RGE_ALLOW_LIVE_LLM", "0").strip().casefold()
    if live not in ("1", "true", "yes"):
        pytest.skip("live staged extract requires RGE_ALLOW_LIVE_LLM=1")
    if os.environ.get("RGE_LLM_MODE", "mock") != "ollama":
        pytest.skip("live staged extract requires RGE_LLM_MODE=ollama")
...
def test_require_live_staged_extract_live_llm_skips_without_opt_in(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("RGE_ALLOW_LIVE_STAGED_EXTRACT_LIVE_LLM", raising=False)
...
def temp_db(tmp_path: Path) -> Path:
    return tmp_path / "live_staged_extract_live.sqlite"
...
def test_staged_source_without_fallthrough_still_uses_auto_mock_fixture(
    temp_db: Path,
    staging_dir: Path,
    ...
    source_id = _ingest_staged_source(temp_db, staging_dir)
    assert (
...
def test_live_staged_fallthrough_blocked_without_staged_live_gate(
    temp_db: Path,
    staging_dir: Path,
    staged_db_with_candidate: Path,
) -> None:
    ...
    with patch.dict(
        ...
        os.environ.pop("RGE_ALLOW_LIVE_STAGED_EXTRACT_LIVE_LLM", None)
        exit_code = main(
            [
                "extract-claims",
                "--source",
...
def test_live_staged_fallthrough_blocked_on_default_graph_db(
    temp_db: Path,
    staging_dir: Path,
    staged_db_with_candidate: Path,
) -> None:
...
def test_mocked_live_staged_fallthrough_persists_validated_claims(
    temp_db: Path,
    staging_dir: Path,
    staged_db_with_candidate: Path,
) -> None:
    ...
    with patch.dict(
        ...
        with patch(
            ...
            with patch(
                ...
                return_value=_StubStagedOllamaClient(),
            ):
                exit_code = main(
                    [
                        "extract-claims",
...
@pytest.fixture()
def live_staged_extract_live_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("RGE_ALLOW_LIVE_STAGED_EXTRACT_LIVE_LLM", "1")
...
def test_live_openalex_discover_fetch_ingest_live_extract(
    live_staged_extract_live_env: None,
    temp_db: Path,
    staging_dir: Path,
    ...
    assert extract_payload.get("live_staged_fallthrough") is True
