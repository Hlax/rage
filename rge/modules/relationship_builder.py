...
def is_staged_fetch_spine_source(source: Any | None) -> bool:
    """Public helper for staged OpenAlex ingest title heuristic."""
    ...
    *,
    ...
    if live_staged_build_fallthrough:
        ...
        if not is_staged_fetch_spine_source(source_record):
            raise ValueError(
                "live_staged_build_fallthrough requires staged OpenAlex ingest source title "
                "(human-ai co-creativity / songwriting marker)."
            )
        model_client = client or get_model_client(cfg, mode="ollama")
    elif live_manual_relationship_fallthrough:
...
def assert_live_staged_build_live_env(
    config: Any | None = None,
    *,
    ...
    if allow not in ("1", "true", "yes"):
        raise LiveExtractionWriteError(
            f"{command} live staged build fallthrough requires "
            "RGE_ALLOW_LIVE_STAGED_BUILD_LIVE_LLM=1."
        )
    return assert_live_probe_env(config, command=command)


def build_relationships_staged_live_fallthrough(
    conn: Any,
    source_id: str,
    *,
    config: Any | None = None,
    ...
    """Live Ollama relationship drafting for staged OpenAlex ingest sources (no auto-mock)."""
    from rge.db.repositories import (
        RelationshipEvidenceRepository,
        RelationshipRepository,
        SourceRepository,
    ...
    cfg = assert_live_staged_build_live_env(config, command="build-relationships")
    health = assert_ollama_health(cfg) if not skip_health_check else {}
