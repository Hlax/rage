...
**Live staged network proofs** (operator opt-in; tickets 167–168): pytest proofs on
real OpenAlex HTTP with temp DB paths. **Not** run in CI or default `pytest`
(collection excludes `live_network`; see `pyproject.toml`). No live LLM; proofs stop
before `extract-claims`.
