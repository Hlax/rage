...
- **Arbitrary-source pipeline:** partial — Phase 3 staged mock spine fixture-proven through
  ...
  (ticket-187) and rank-2 discover→…→generate-run-report (ticket-190) via
  `pytest -m live_network` (not CI); single-command live
  `research run --staged-spine` not proven; evidence DB NM-4 proven; default graph
  synthnote checksum-mock; `research run` without `--fixture-mode` remains
  `not_implemented`.
- **Cloud providers:** deferred (ticket-059).

**Live staged network proofs** (operator opt-in; tickets 167–190): real OpenAlex HTTP
...
discover through generate-run-report with second-candidate mock fixtures. Temp DB/output
only — no public export. Does **not** prove single-command live
`research run --fixture-mode --staged-spine`. Env gates: `RGE_ALLOW_SOURCE_NETWORK=1`,
...
(discover→fetch→ingest→extract→link→build→detect→reconcile→generate-run-report), or
`RGE_ALLOW_LIVE_STAGED_RANK2=1`
(rank-2 discover→fetch→ingest→second-candidate mock extract→…→generate-run-report).
